...
async function main(): Promise<void> {
  ...
  assert.ok(call('agents:list').some((agent: { id: string }) => agent.id === 'codex'))
  ...
  agentProcesses.result(task.id, { id: tracker.items[0].id, status: 'complete', checklist: [true], evidence: 'Test passed' })
  await tick()
  ...
  agentProcesses.result(task.id, { id: tracker.items[1].id, status: 'complete', checklist: [true], evidence: 'Test passed' })
  await tick()
  ...
  await tick()
  agentProcesses.result(invalid.id, { items: [] })
  await tick()
  ...
  await tick()
  agentProcesses.result(cancelled.id, { items: [issue, { ...issue, key: 'second', dependencies: ['first'] }] })
  await tick()
  const currentIssue = trackers.get(cancelled.id)!.items[0]
  const startsBeforeCancel = agentProcesses.starts.length
  agentProcesses.result(cancelled.id, { id: currentIssue.id, status: 'complete', checklist: [true], evidence: 'Test passed' })
