...
export function registerProjectHandlers(ipc: RendererIpc, {
  ...
  ipc.handle('projects:reveal', (_event, id) => {
    const project = store.getProjects().find((item) => item.id === id)
    if (!project) throw new Error('Project not found')
    return shell.openPath(project.path)
  })

  ipc.handle('projects:git-status', async (_event, id: string) => {
    const project = store.getProjects().find((item) => item.id === id)
    if (!project) throw new Error('Project not found')
    return gitDelivery.status(project.path)
  })

  ipc.handle('projects:git-init', async (_event, id: string) => {
    const project = store.getProjects().find((item) => item.id === id)
    if (!project) throw new Error('Project not found')
    return gitDelivery.init(project.path)
  })

  ipc.handle('projects:branches', async (_event, id) => {
    const project = store.getProjects().find((item) => item.id === id)
    if (!project) throw new Error('Project not found')
