...
test('Escape dismisses the unified picker before the new-task modal', async ({ page }, testInfo) => {
  await page.goto(fixture)
  await page.getByRole('button', { name: 'New task', exact: true }).click()
  const modal = page.getByRole('dialog', { name: 'Start new task', exact: true })
  await modal.getByRole('textbox', { name: 'Task prompt' }).fill('Keep the modal draft')
  const more = modal.getByRole('button', { name: 'More task options', exact: true })
  const provider = modal.getByRole('button', { name: /^(Choose a model|Model:)/ })
  ...
  await page.screenshot({ path: testInfo.outputPath('provider-picker-modal.png') })
  await page.keyboard.press('Escape')
  ...
  await expect(modal).toBeVisible()
  const model = modal.getByRole('button', { name: 'Model: GPT 5', exact: true })
  await model.click()
  await page.mouse.click(5, 5)
  await expect(model).toBeFocused()
  await expect(modal.getByRole('textbox', { name: 'Task prompt' })).toHaveValue('Keep the modal draft')
  await page.keyboard.press('Escape')
