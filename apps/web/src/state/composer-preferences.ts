...
import type { ComposerPreferences, TaskReviewPolicy } from '@anvil/protocol/types'
...
interface ComposerPreferencesState extends ComposerPreferences {
  ...
  setReviewPolicy: (reviewPolicy: TaskReviewPolicy) => void
}

export function hydrateComposer(workspaceId: string, preferences: ComposerPreferences): void {
  useComposerPreferences.setState({ workspaceId, ...preferences, reviewPolicy: preferences.reviewPolicy ?? 'review_each_issue', saveError: null })
}
...
export const useComposerPreferences = create<ComposerPreferencesState>((set, get) => {
  const save = (patch: Partial<ComposerPreferences>): void => {
    ...
    const { agentId, modelsByAgent, reasoningByAgentModel, reviewPolicy } = get()
    void enqueueWorkspaceRequest(() => window.anvil.workspaces.setPreferences(workspaceId, {
      composer: { agentId, modelsByAgent, reasoningByAgentModel, reviewPolicy }
    })).catch((error: unknown) => {
      if (get().workspaceId === workspaceId) set({ saveError: error instanceof Error ? error.message : 'Could not save composer preferences' })
    })
  }
  return {
    workspaceId: null, agentId: '', modelsByAgent: {}, reasoningByAgentModel: {}, reviewPolicy: 'review_each_issue', saveError: null,
    ...
    setReviewPolicy: (reviewPolicy) => save({ reviewPolicy })
  }
})
