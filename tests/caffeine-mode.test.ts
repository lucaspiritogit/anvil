...
test('toggles sleep prevention for running, paused, concurrent and deleted tasks', () => {
  ...
  try {
    ...
    handlers.get('settings:set')!(rendererEvent, { workspaceId: 'default', patch: { caffeineMode: true } })
    expect(() => handlers.get('settings:set')!(rendererEvent, { workspaceId: 'default', patch: { caffeineMode: 'true' } })).toThrow(/Invalid IPC request/)
...
test('sleep prevention follows running task owners across workspace selection', () => {
  const { store, active, task } = setupCaffeine()
  store.addProject({ id: 'project', name: 'Test', path: testHome, createdAt: 0, monthlyTokenLimit: null, monthlyCostLimitUsd: null, finishOnPush: false, gitPlatform: 'github' })
  ...
  store.addTask({ ...task, id: 'work-task', workspaceId: work.id })
  expect(active.size).toBe(0)
  store.setSettings({ caffeineMode: true }, work.id)
  expect(active.size).toBe(1)
  store.selectWorkspace('default')
  store.setSettings({ caffeineMode: false }, 'default')
  expect(active.size).toBe(1)
  store.updateTask('work-task', { status: 'succeeded' })
  expect(active.size).toBe(0)
})
