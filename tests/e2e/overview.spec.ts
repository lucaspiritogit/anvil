...
test('overview keeps the floating composer visible in a short window', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 900, height: 600 })
  await page.goto(fixture)
  const main = page.getByRole('main')
  await expect(main.getByRole('heading', { name: 'Usage this month' })).toHaveCount(0)
  await expect(main.getByRole('heading', { name: /Describe what to build/ })).toHaveCount(0)
...
test('Send dispatches the prompt and selected model to the current project', async ({ page }) => {
  ...
  const taskButton = page.getByRole('button', { name: 'Open task: Build a project search', exact: true })
...
for (const modifier of ['Meta', 'Control']) {
  test(`${modifier}+Enter dispatches once, while Shift+Enter adds a newline`, async ({ page }) => {
    ...
    await expect(page.getByTestId('project-overview')).toBeVisible()
    await expect(composer).toBeVisible()
    await expect(page.getByRole('button', { name: 'Open task: Build search', exact: true })).toHaveCount(1)
    const startedTasks = await page.evaluate(async () => (await window.anvil.tasks.list()).filter((task) => task.id.startsWith('started-')))
    expect(startedTasks).toHaveLength(1)
...
for (const [platform, modifier] of [['darwin', 'Meta'], ['linux', 'Control'], ['win32', 'Control']]) {
  test(`${platform} new-task shortcut focuses the composer without submitting or clearing its draft`, async ({ page }) => {
    ...
    await expect.poll(() => workbenchPrompt.evaluate((element) => [element.selectionStart, element.selectionEnd])).toEqual([draft.length, draft.length])
    await expect(page.getByRole('log', { name: 'Task output' })).toHaveCount(0)
