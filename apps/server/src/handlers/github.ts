...
import { isGitHubPullRequestUrl } from '@anvil/protocol/github-repository'
import type { GitHubCredentialStatus, PullRequestInfo, PullRequestPreview } from '@anvil/protocol/types'
...
export function registerGitHubHandlers(ipc: HandlerRegistry, {
  ...
  ipc.handle('github:set-token', async (value: string): Promise<GitHubCredentialStatus> => {
  ...
  ipc.handle('github:pr-preview', async (taskId: string): Promise<PullRequestPreview> => {
    const { task, project, branchName } = requireTask(taskId)
    return pullRequests(task.workspaceId).preview(project.path, branchName)
  })
  ipc.handle('github:open-pr', async (input): Promise<PullRequestInfo> => {
  ...
  ipc.handle('github:draft-pr-field', async (input): Promise<string> => {
  ...
  ipc.handle('github:open-pr-url', (url: string): string => {
    if (!isGitHubPullRequestUrl(url) || new URL(url).href !== url) throw new Error('Invalid GitHub PR URL.')
    return url
  })
}
