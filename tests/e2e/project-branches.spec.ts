...
test('branch keyboard navigation skips in-use branches and exposes long project and branch names', async ({ page }, testInfo) => {
  ...
  // Re-enter the overview so the project list and branch picker refresh.
  await page.getByRole('button', { name: 'New task', exact: true }).click()
  const modal = page.getByRole('dialog', { name: 'Start new task', exact: true })
  const branch = modal.getByRole('button', { name: 'Project branch', exact: true })
  await expect(branch).toHaveAccessibleDescription(longBranch)
  const project = modal.getByTestId('composer-project-name')
...
test('detached HEAD can choose a branch and non-Git projects retain their name', async ({ page }) => {
  ...
  await expect(modal.getByTestId('composer-project-name')).toBeVisible()
})
...
test('branch loading and discovery errors recover on refresh', async ({ page }) => {
  ...
  await expect(modal.getByRole('alert')).toHaveText('Repository is temporarily unavailable')
  await page.evaluate(() => {
