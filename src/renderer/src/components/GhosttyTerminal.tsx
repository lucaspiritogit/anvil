...
export function GhosttyTerminal({ sessionId, className, visible = true, onExit }: {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const input = term.onData((data) => window.anvil.terminals.write({ sessionId, data }))
      const resize = term.onResize(({ cols, rows }) => window.anvil.terminals.resize({ sessionId, cols: Math.min(500, Math.max(2, cols)), rows: Math.min(300, Math.max(1, rows)) }))
      let attached = false
      let sequence = 0
