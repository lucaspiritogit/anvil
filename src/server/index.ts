...
async function main(): Promise<void> {
  ...
  const http = createAnvilHttpServer({
    invoke: (channel, input) => {
      ...
      return runtime.invoke(channel, input)
    },
    subscribeAll: (listener) => {
      listeners.add(listener)
      return () => { listeners.delete(listener) }
    }
  }, { version, rendererOrigin: process.env.ANVIL_RENDERER_ORIGIN ?? 'http://localhost:5173' })
  let closing = false
  ...
  try {
    ...
    runtime = createAnvilRuntime({
      dataDirectory: resolveAppDataDirectory(homedir(), process.env.ANVIL_PACKAGED === '1', process.env.ANVIL_DATA_DIR),
