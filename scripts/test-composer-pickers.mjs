...
try {
  ...
  const modal = page.getByRole('dialog', { name: 'Start new task', exact: true })
  const nestedProvider = modal.getByRole('button', { name: /^(Choose a model|Model:)/ })
  await nestedProvider.click()
  await expect(search).toBeFocused()
  await page.screenshot({ path: join(screenshots, 'nested-provider.png') })
  await page.keyboard.press('Escape')
  await expect(nestedProvider).toBeFocused()
  ...
  console.log(`Electron checks passed: real checkout success/failure, long names, provider keyboard selection, nested Escape, focus restoration. Screenshots: ${screenshots}`)
