...
function prepareNodeSqlite() {
  ...
  const destination = join(__dirname, '../node_modules/.anvil-node-native', fingerprint, 'better-sqlite3')
  ...
  console.log(`Preparing Node SQLite for Node ${process.versions.node} (ABI ${process.versions.modules})…`)
  ...
  if (result.status !== 0) {
    throw new Error('Could not build Node SQLite. Install Python and the C/C++ build tools for this server, then retry.')
  }
  openBinding()
  console.log('Node SQLite is ready.')
  return binding
}
