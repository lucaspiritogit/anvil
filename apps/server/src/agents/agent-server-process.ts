...
/** Wait for the server and terminate tools even if they create separate groups. */
export function closeAgentServer(child: ChildProcess, closed: Promise<void>): Promise<void> {
  return closeProcessTree(child.pid, closed, (force) => killAgentServer(child, force))
}
