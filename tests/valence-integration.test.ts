...
test('initialization and claiming roll back together and retries retain task ownership', () => {
  ...
  const blockedState = store.getTaskExecution(taskId)!
  expect(issues.resume(taskId)).toMatchObject({ phase: 'recovering', currentIssueId: child.id })
  expect(tracker.get(child.id).status).toBe('working')
  issues.rollbackResume(taskId, blockedState)
  expect(store.getTaskExecution(taskId)).toEqual(blockedState)
  expect(tracker.get(child.id).status).toBe('blocked')
  issues.resume(taskId)
  expect(tracker.get(child.id).status).toBe('working')
  expect(() => issues.finishRecovery(taskId)).toThrow(/not submitted for review/)
