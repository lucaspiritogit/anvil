...
export function CopyableText({ label, value }: { label: string; value: string }): JSX.Element {
  ...
  return (
    <span className="relative inline-flex min-w-0 items-center gap-1.5">
      ...
      <span role="status" className={feedback ? 'absolute right-0 bottom-full z-10 mb-1 w-max max-w-64 rounded-md border border-line bg-hover px-2 py-1 text-xs text-fg shadow-lg' : 'sr-only'}>
        {feedback}
      </span>
    </span>
  )
