...
createInterface({ input: process.stdin }).on('line', (line) => {
  ...
  if (method === 'session/prompt' || method === 'turn/start') {
    ...
    if (['child', 'detached-child'].includes(session.prompt)) {
      process.on('SIGTERM', () => {})
      // A tool can create its own process group and ignore termination.
      const child = spawn(process.execPath, ['-e', 'process.on("SIGTERM", () => {}); console.log("ready"); setInterval(() => {}, 1000)'], { detached: session.prompt === 'detached-child' && process.platform !== 'win32', stdio: ['ignore', 'pipe', 'ignore'] })
