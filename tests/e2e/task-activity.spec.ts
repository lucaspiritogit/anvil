...
test('activity breathes while waiting and follows thinking, tool calls, results, and messages', async ({ page }, testInfo) => {
  ...
  await expect(activity.locator('span').first()).toHaveCSS('animation-name', 'breathe')
  await expect(activity.locator('span').first()).toHaveCSS('animation-iteration-count', 'infinite')
  const animationTime = await activity.locator('span').first().evaluate((element) => Number(element.getAnimations()[0].currentTime))
  await expect.poll(() => activity.locator('span').first().evaluate((element) => Number(element.getAnimations()[0].currentTime))).toBeGreaterThan(animationTime)
...
test('reduced motion keeps the activity text without animation', async ({ page }) => {
  ...
  const activity = page.getByRole('status', { name: 'Agent activity' })
  await expect(activity).toHaveText('Working…')
  await expect(activity.locator('span').first()).toHaveCSS('animation-name', 'none')
