...
for (const viewport of [{ width: 1440, height: 900 }, { width: 900, height: 500 }]) {
  test(`issue cards, keyboard, live details and tab regressions at ${viewport.width}`, async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('textbox', { name: 'Task prompt', exact: true })).toBeHidden()
    ...
    for (const issue of snapshot.children) await expect(panel.getByRole('button', { name: issue.title, exact: true })).toBeVisible()
    ...
    await expect(panel.getByRole('button', { name: 'Issue 0', exact: true })).toHaveAttribute('aria-expanded', 'true')
