...
if (scenario === 'image-models' || scenario === 'image-models-text') {
  console.log('provider/model')
  console.log(JSON.stringify({ capabilities: { input: { image: scenario === 'image-models' } } }, null, 2))
  process.exit(0)
}
...
const modeConfig = () => [{
  id: 'mode', name: 'Session Mode', category: 'mode', type: 'select', currentValue: planSelected ? 'plan' : 'build',
  options: scenario === 'read-only-config-grouped'
    ? [{ group: 'agents', name: 'Agents', options: [{ value: 'plan', name: 'Plan' }] }]
...
const sessionConfig = () => hasModeConfig ? modeConfig() : ['limited-effort', 'removed-effort'].includes(scenario)
  ? effortConfig(['medium'])
...
function finish() {
  if (scenario === 'read-only-config-draft') {
    text(draftField === 'title' ? 'Improve task review' : 'Improve the review layout. Validation was not reported.')
...
createInterface({ input: process.stdin }).on('line', (line) => {
  ...
  if (message.id === 'permission') {
    ...
    if (scenario === 'deny-only' || hasModeConfig) {
  ...
  if (message.method === 'initialize') {
    ...
    if (hasModeConfig && message.params.configId === 'mode') {
      if (message.params.value !== 'plan') process.exit(13)
      if (scenario === 'read-only-config-rejected') return send({ id: message.id, error: { code: -32602, message: 'Plan mode rejected' } })
      planSelected = true
      return respond(message.id, { configOptions: modeConfig() })
    }
    ...
    promptId = message.id
    if (scenario === 'read-only-config-draft') {
      if (!modelSelected) process.exit(11)
      const prompt = message.params.prompt[0].text
      if (!prompt.startsWith('Write only the GitHub pull request ')) process.exit(11)
      draftField = prompt.includes('pull request title') ? 'title' : 'description'
    } else if ((!modelSelected && scenario !== 'session-effort') || message.params.prompt[0].text !== 'Implement the issue') process.exit(11)
