...
try {
  ...
  await expect(main.getByRole('button', { name: 'Save', exact: true })).toBeInViewport()
  ...
  await expect(main.getByTestId('project-overview')).toHaveCSS('background-color', 'rgb(18, 52, 86)')
  ...
  await main.getByRole('button', { name: 'Save', exact: true }).click()
  await expect(main.getByText('Saved', { exact: true })).toBeVisible()
  await expect(main.getByRole('button', { name: 'Close', exact: true })).toBeInViewport()
  ...
  if (process.platform === 'darwin' && !process.argv.includes('--manual')) {
    ...
    await expect(restored.getByRole('navigation', { name: 'Settings sections' })).toBeVisible()
  ...
  console.log('Electron settings passed: sidebar, native menu, keyboard, one window, restore/focus, save/close/reopen, collapsed sidebar restored, composer draft retained, display/project/memory preferences, shortcut recording, small-window layout.')
