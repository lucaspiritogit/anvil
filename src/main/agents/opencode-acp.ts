    ...
    try {
      ...
      if (input.resumeSessionId) {
        ...
        if (input.readOnly) {
          ...
          const hasPlan = mode?.type === 'select' && mode.options
            .flatMap((option) => 'group' in option ? option.options : [option])
            .some((option) => option.value === 'plan')
          if (!mode || !hasPlan) throw new Error('This OpenCode server does not offer read-only plan mode for PR drafting.')
          const selection = await request(connection.rpc.setSessionConfigOption({ sessionId, configId: mode.id, value: 'plan' }))
