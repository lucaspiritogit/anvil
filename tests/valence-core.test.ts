...
test('priority then creation ordering, completion evidence, transitions and array replacement', () => {
  ...
  expect(a.update(urgent.id, { title: 'Changed' })).toMatchObject({ title: 'Changed', status: 'working' })
...
test('review status gates completion behind developer approval', () => {
  ...
  expect(a.update(first.id, { title: 'Changed' })).toMatchObject({ title: 'Changed', status: 'review' })
