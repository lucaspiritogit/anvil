...
test('default provider uses the custom picker and persists the choice', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Save', exact: true }).click()
  await expect(page.getByText('Saved', { exact: true })).toBeVisible()
  expect((await page.evaluate(() => window.anvil.settings.get())).defaultAgentId).toBe('opencode')
})

test('workspace settings restore appearance and clear unsaved settings drafts on switch', async ({ page }) => {
