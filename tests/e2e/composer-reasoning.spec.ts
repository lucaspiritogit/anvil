...
async function configureModels(page: Page): Promise<void> {
  ...
  await chooseProvider(page.getByRole('button', { name: /^(Choose a model|Model:)/ }), 'opencode')
}
...
test('model selection refreshes available efforts and remembers a valid choice per model', async ({ page }, testInfo) => {
  ...
  await page.reload()
  await expect(reasoning).toHaveValue('max')
  await chooseProvider(composer.getByRole('button', { name: /^(Choose a model|Model:)/ }), 'codex')
  await expect(reasoning).toHaveValue('high')
...
for (const agentId of ['codex', 'opencode']) {
  test(`${agentId} submits the discovered effort immediately after delayed discovery`, async ({ page }) => {
    ...
    const composer = page.getByRole('form', { name: 'Start a task' })
    await chooseProvider(composer.getByRole('button', { name: /^(Choose a model|Model:)/ }), agentId)
