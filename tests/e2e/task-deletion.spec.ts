...
test('right-click deletion requires confirmation and removes the selected task', async ({ page }) => {
  ...
  await expect(dialog).toContainText('its output and review comments')
  await expect(dialog).toContainText('Project files and Git branches will not be deleted.')
  const warning = dialog.locator('#delete-task-warning')
  await expect(warning).toHaveText('This permanently removes the task and its related records from database storage.')
  await expect(warning).toHaveClass(/text-xs.*text-danger/)
  await expect(warning.locator('svg')).toBeVisible()
  await expect(dialog).toHaveAttribute('aria-describedby', 'delete-task-description delete-task-warning')
