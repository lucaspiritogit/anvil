...
test('task usage shows cached input and explains cumulative request totals', async ({ page }, testInfo) => {
  ...
  const statistics = main.getByRole('group', { name: 'Task statistics' })
  const cached = statistics.locator('[title="638.2K cached input"]')
  await expect(cached.getByText('Cached', { exact: true })).toBeVisible()
  await expect(cached.getByText('638.2K', { exact: true })).toBeVisible()
  await expect(statistics.getByText(/^688\.8K\s*in$/)).toBeVisible()
  await expect(statistics.getByText(/^3\.6K\s*out$/)).toBeVisible()
  const tokens = statistics.locator('[title*="688,809 input"]')
...
test('mobile task details disclose complete live usage and copy controls', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 390, height: 720 })
  await page.goto('/tests/e2e/fixture/?scenario=output&taskUsage=1&steering=1')
  const main = page.getByRole('main')
  const disclosure = main.getByRole('button', { name: 'Details', exact: true })
  const details = disclosure.locator('..')

  await expect(main.getByRole('heading', { level: 1 })).toHaveText('Layout test task')
  await expect(main.getByLabel('Task status')).toBeVisible()
  await expect(details).not.toHaveAttribute('open', '')
  await expect(details.getByLabel('Task workspace')).toBeHidden()
  await expect(main.getByRole('group', { name: 'Task statistics' })).toBeHidden()

  await disclosure.click()
  await expect(details).toHaveAttribute('open', '')
  await expect(details.getByLabel('Task workspace')).toHaveText('default')
  await expect(details.getByText('task-model', { exact: false })).toBeVisible()
  const statistics = main.getByRole('group', { name: 'Task statistics' })
  await expect(statistics.getByText('Elapsed', { exact: true })).toBeVisible()
  await expect(statistics.getByText('638.2K', { exact: true })).toBeVisible()
  await expect(statistics.getByText(/^688\.8K\s*in$/)).toBeVisible()
  await expect(statistics.getByText(/^3\.6K\s*out$/)).toBeVisible()
  await expect(statistics.getByText('Cost', { exact: true })).toBeVisible()
  await expect(statistics.getByText('$0.0000', { exact: true })).toBeVisible()
  await expect(main.getByRole('button', { name: 'Copy branch name' })).toBeVisible()
  await expect(main.getByRole('button', { name: 'Copy task ID' })).toBeVisible()
  await main.getByRole('button', { name: 'Copy task ID' }).click()
  await expect(main.getByRole('status').filter({ hasText: 'Copied task ID' })).toBeVisible()
