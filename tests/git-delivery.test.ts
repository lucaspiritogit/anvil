...
async function main(): Promise<void> {
  ...
  try {
    ...
    const mergePreview = await manager.getMergePreview(repo, first.branchName)
    assert.equal(mergePreview.commitCount, 2, 'Merge preview includes the starting snapshot absent from the user branch')
    assert.equal(git(repo, 'status', '--porcelain=v1'), originalStatus, 'Merge preview does not touch local changes')
