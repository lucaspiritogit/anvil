import type { PullRequestInfo, PullRequestPreview } from '@anvil/protocol/types'
...
export class GitHubPullRequests {
  ...
  async open(projectPath: string, branchName: string, preview: PullRequestPreview, title: string, description: string, check: () => void = () => {}): Promise<PullRequestInfo> {
    ...
    check()
    await this.gitDelivery.pushPullRequestBranch(projectPath, preview, check)
    check()
    try {
